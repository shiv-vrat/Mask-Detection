...
while True:
    ...
    rects=detector.detectMultiScale(frame,scaleFactor=1.1,minNeighbors=5,minSize=(30,30),flags=cv2.CASCADE_SCALE_IMAGE)
    ...
    for (X,Y,W,H) in rects:
        ...
        if notSmiling>smiling:
            label="No MASK {}%".format("%.2f" %notSmiling)
        else:
            label="MASK {}%".format("%.2f" %smiling)
